...
const App = () => {
  ...
  return (
    <Container>
      ...
      />
      {isLoading ? <Loader /> : results?.length ? results?.map((result: Notif) => <Card type={result.type} asset={result.data.unit} amount={result.data.amount} name={result.data.name} receiver={result.data.to} sender={result.data.from} key={result.id} />) : <Error message='No data found' onClick={() => setSearchText("")} />}
    </Container>
  )
}
